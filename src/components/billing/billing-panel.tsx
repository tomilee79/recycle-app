...
const expenseFormSchema = z.object({
  ...
  category: z.enum(['유류비', '정비비', '통행료', '기타']),
...
export default function BillingPanel() {
  ...
  const handleExpenseStatusChange = (id: string, newStatus: ExpenseStatus) => {
    ...
    toast({
      title: '상태 변경 완료',
      description: `비용 항목의 상태가 '${expenseStatusMap[newStatus]}'(으)로 변경되었습니다.`,
    });
  };
            ...
            <Card className="shadow-lg">
                ...
                <ChartContainer config={chartConfig} className="h-[400px] w-full">
                    ...
                    <ComposedChart data={localizedReportData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                        ...
                        <YAxis yAxisId="right" orientation="right" stroke="hsl(var(--primary))" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `${(value / 10000).toFixed(1)}`} />
                    ...
                    {paginatedSettlementData.map((item) => (
                        ...
                        <TableCell>
                            ...
                            <DropdownMenuContent align="end">
                                {(Object.keys(settlementStatusMap) as SettlementStatus[]).filter(status => status !== item.status).map(status => (
                                ...
                                return (
                                    <TableRow key={item.id} onClick={() => openExpenseDialog(item)} className="cursor-pointer">
                                        ...
                                        <TableCell onClick={(e) => e.stopPropagation()}>
                                            ...
                                            <DropdownMenuContent align="end">
                                                {(Object.keys(expenseStatusMap) as ExpenseStatus[]).filter(status => status !== item.status).map(status => (
                                                <DropdownMenuItem key={status} onSelect={() => handleExpenseStatusChange(item.id, status)}>{expenseStatusMap[status]}으로 변경</DropdownMenuItem>
                                                ))}
                                            </DropdownMenuContent>
                                            </DropdownMenu>
                                        </TableCell>
            ...
            <DialogContent className="sm:max-w-lg">
                ...
                <form onSubmit={expenseForm.handleSubmit(onExpenseSubmit)} className="space-y-4 mt-4">
                    <FormField control={expenseForm.control} name="date" render={({ field }) => (
                        <FormItem className="flex flex-col"><FormLabel>지출일</FormLabel>
